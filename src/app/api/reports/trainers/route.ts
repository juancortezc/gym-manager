...
export async function GET(request: NextRequest) {
  try {
    ...
    const trainerReports = trainers.map(trainer => {
      const sessions = trainer.workSessions
      
      // Calculate totals
      const totalHours = sessions.reduce((sum, session) => sum + (session.totalHours || 0), 0)
      const totalPayment = totalHours * trainer.hourlyRate
    ...
    // Filter out trainers with no sessions and sort by total payment descending
    const activeTrainerReports = trainerReports
      .filter(report => report.sessionCount > 0)
      .sort((a, b) => b.totalPayment - a.totalPayment)

    // Calculate summary statistics
    const summary = {
      totalTrainers: activeTrainerReports.length,
      totalHours: activeTrainerReports.reduce((sum, t) => sum + t.totalHours, 0),
      totalPayments: activeTrainerReports.reduce((sum, t) => sum + t.totalPayment, 0),
      totalSessions: activeTrainerReports.reduce((sum, t) => sum + t.sessionCount, 0)
    }
